...
if ($socid < 1){
	$retour['error']++;
	$retour['message'] .= $langs->trans("CustomerNotValid").'<br>';
}
...
if (!$retour['error'])
{
	...
	if($conf->global->BANKIMPORTAPI_GENERATE_UNIQUE_REF == 1){
		$ref_customer = $ref_element.' (#'.substr($now,6).')';
	}else{
		$ref_customer = $ref_element;
	}
	
	
	$object->socid = $socid;
	$object->type            = Facture::TYPE_STANDARD;
	$object->ref             = $ref_customer;
	$object->date            = $datefacturation;
	$object->ref_client      = $ref_customer;
	...
	if($idFac > 0){
		...
		$pu = round($montant_ttc/(1+($tva_tx/100)),2);
		$qty = 1;
		$type = 1;
	...
	if (!$retour['error'])
	{
		...
		$bank_line_id=$paiement->addPaymentToBank($user, 'payment', '(CustomerInvoicePayment)', $bankId, '', '');
