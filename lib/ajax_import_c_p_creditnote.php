...
$datefacturation = (!empty($formTab['datecreditnote']) && strlen($formTab['datecreditnote']) == 10)?convertDatePost($formTab['datecreditnote']):'';
$projectid = (empty($conf->projet->enabled))?null:$formTab['projectid'];
$label = $formTab['label'];
$montant_ht = $formTab['montant_ht'];
$montant_tva = $formTab['montant_tva'];
$socid = $formTab['socid_ajax_provider_invoices'];
$releve = $formTab['releve'];
$mode_reglement_id = (!empty($mode_reglement_id))?$mode_reglement_id:$formTab['mode_reglement_id'];
$sourceinvoice = $formTab['fac_avoir'];

/*
$retour['error']++;
$retour['message'] .= '<br>idline: '.$id_line;
$retour['message'] .= '<br>date_op: '.$date_op;
$retour['message'] .= '<br>date_valeur: '.$date_valeur;
$retour['message'] .= '<br>label_banque: '.$label_banque;
$retour['message'] .= '<br>montant_ttc: '.$montant_ttc;
$retour['message'] .= '<br>mode_reglement_id: '.$mode_reglement_id;
$retour['message'] .= '<br><br>ref_element: '.$ref_element;
$retour['message'] .= '<br>datefacturation: '.$datefacturation;
$retour['message'] .= '<br>projectid: '.$projectid;
$retour['message'] .= '<br>label: '.$label;
$retour['message'] .= '<br>montant_ht: '.$montant_ht;
$retour['message'] .= '<br>montant_tva: '.$montant_tva;
$retour['message'] .= '<br>socid: '.$socid;
$retour['message'] .= '<br>releve: '.$releve;
$retour['message'] .= '<br>mode_reglement_id: '.$mode_reglement_id;
$retour['message'] .= '<br>sourceinvoice: '.$sourceinvoice;
*/
...
if (empty($sourceinvoice) || $sourceinvoice < 0){
	$retour['error']++;
	$retour['message'] .= $langs->trans("SourceInvoiceSHouldBeDefined").'<br>';
}
...
if (!$retour['error'])
{
	...
	if(!$retour['error']){
		...
		$newvalue = price2num($montant_ttc, 'MT');

		$tab[$idFac] = - abs($newvalue);
