...
<!---------------------------------------------------------------->
<!--Form import salaries-->
<!---------------------------------------------------------------->
<div id="dialog_c_p_salary" class="dialog_box" style="display:none;" title="<?php print $langs->trans("CreateAndPayeSalary"); ?>">
	...
	<table>
		<input type="hidden" name="url_ajax" value="lib/ajax_import_c_p_salary.php">
		
		<tr>
			<td class="titlefield"><span class="fieldrequired"><?php print $langs->trans("SalaryRef"); ?></span></td>
		...
		<tr>
			...
			<td>
				<?php $formproject->select_projects((empty($conf->global->PROJECT_CAN_ALWAYS_LINK_TO_ALL_SUPPLIERS) ? $societe->id : -1), $conf->global->BANKIMPORTAPI_ID_PROJET_DEFAUT, 'projectid', 0, 0, 1, 1, 0, 0, 0, '', 0, 0, 'minwidth200 maxwidth500'); ?>
			</td>
		</tr>
		<?php }?>
		
		<tr>
			<td class="titlefield"><span class="fieldrequired"><?php print $langs->trans("Employee"); ?></span></td>
			<td>
			<div class="tagtd maxwidthonsmartphone">
			<?php
				$noactive=0;	// We keep active and unactive users
				print $form->select_dolusers('', 'fk_user', 1, '', 0, '', '', 0, 0, 0, 'AND employee=1', 0, '', 'minwidth200imp', $noactive);
			?>
			</div>
			</td>
		</tr>
		...
		<tr>
			<td class="titlefield"><span class="fieldrequired"><?php print $langs->trans("PeriodStartDate"); ?></span></td>
			<td>
				<?php
				print $form->selectDate($dateLastDay, 'period_start_salary', 0, 0, 1, 'charge', 1, 0);
				?>
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				<?php
				print $form->selectDate($dateLastDay, 'period_end_salary', 0, 0, 1, 'charge', 1, 0);
				?>
			</td>
		</tr>
...
$( ".button_c_p_salary" ).click(function() {
	
	var idOfDialog = "#dialog_c_p_salary";
	...
	$('#fk_user').find('option[value="-1"]').prop('selected', true);
	$('#select2-fk_user-container').text('');

	
	if(dateEmitted.getDate() < 15){
		
		var dateStart = new Date(dateEmitted.getFullYear(),dateEmitted.getMonth()-1,1);
		
		var dateEnd = new Date(dateEmitted.getFullYear(),dateEmitted.getMonth(),0);

	}else{
		
		var dateStart = new Date(dateEmitted.getFullYear(),dateEmitted.getMonth(),1);
		
		var dateEnd = new Date(dateEmitted.getFullYear(),dateEmitted.getMonth()+1,0);
	...
	$(idOfDialog).find('input[name="period_start_salary"]').val(dateStart);
	
	$(idOfDialog).find('input[name="period_end_salary"]').val(dateEnd);
